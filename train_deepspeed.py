...
from uni_dist.patch_deepspeed import patch_deepspeed
...
backend="deepspeed" #between axonn or deepspeed
overlap_comm=True
...
def get_batch(split):
    ...
    if split == 'train':
        # faster to broadcast val.bin on multiple_nodes
...
cmd_args.deepspeed_config = "./ds_config_z3.json"
...
#parser = argparse.ArgumentParser(description='deepspeed argument parser')
...
if use_pccl:
    patch_deepspeed(torch.cuda.device_count())
...
print_rank0("About to fetch first batch")
# training loop
X, Y = get_batch('train') # fetch the very first batch

MPI.COMM_WORLD.Barrier()
print_rank0("Fetched first batch")
...
while True:
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        #scaler.scale(loss).backward()
            ...
            # check if deepspeed supports grad acc
           # sync_gradients(model, mean=True, vectorize=True)
        #model.step()
    # clip the gradient
    # check if deepspeed supports grad norm
    # if grad_clip != 0.0:
